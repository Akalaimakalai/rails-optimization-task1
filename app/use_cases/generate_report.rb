  ...
  def process_users_objects(report, sessions_by_user)
    @users.each_slice(BANCH_SIZE) do |users|
      users.each do |user|
        ...
        report['usersStats'][user_key]['totalTime'] = "#{time.sum} min."
        report['usersStats'][user_key]['longestSession'] = "#{time.max} min."
        report['usersStats'][user_key]['browsers'] = browsers.join(', ')
        report['usersStats'][user_key]['usedIE'] = any_ie?(browsers)
        report['usersStats'][user_key]['alwaysUsedChrome'] = only_chrome?(browsers)
